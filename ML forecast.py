...
model, res = un_ml_model(n_in = n_in, n_out = n_out, 
                         n_nodes = n_nodes, a_f = a_f, l1 = l1, l2 = l2, 
                         n_epoches = n_epoches, n_batch = n_batch, 
                         train_x = train_x, train_y = train_y, val_x = val_x, val_y = val_y)
...
for ind in all_x.index:
    xxx.loc[ind,"fcs"] = np.array(rescale(model.predict(all_x.loc[ind:ind,:], verbose = 0)[0][n_out-1],
                                 min_dt=min_dt, max_dt=max_dt))
...
for ind in all_x.index:
    xxx.loc[ind,"fcs"] = rescale(model.predict(all_x.loc[ind:ind,:], verbose = 0)[n_out-1],
                                 min_dt=min_dt, max_dt=max_dt)
...
plt.pyplot.plot(res.history['loss'])
plt.pyplot.plot(res.history['val_loss'])


# In[ ]:


pred=pd.DataFrame(data=model.predict(all_x, verbose=0))
pred.index=all_y.index

plt.pyplot.plot(pred)
plt.pyplot.plot(all_y)
...
plt.pyplot.plot(xxx[colname])
plt.pyplot.plot(xxx["fcs"])
...
data_csv.to_clipboard(excel=True, float_format=CommaFloatFormatter())
